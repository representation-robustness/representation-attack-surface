#!/usr/bin/env python3
"""Aggregate exp12 (VulGNN-WithId) 5-seed results into mean ± std JSON."""
...
per_seed = {}
for s in SEEDS:
    p = DEVIGN / f"vulgnn_withid_seed{s}_results.json"
    if p.exists():
...
agg = {"n_seeds": len(per_seed), "seeds": list(per_seed.keys()),
       "model": "vulgnn_withid", "note": "VulGNN + CodeBERT function-level embedding"}
for cond in CONDS:
...
out = DEVIGN / "vulgnn_withid_multiseed_results.json"
