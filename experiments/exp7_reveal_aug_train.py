#!/usr/bin/env python3
"""
Exp 7b: REVEAL-Aug — augmented training for robustness.

Trains REVEAL on originals + obf_deadcode + obf_controlflow training data
(3× augmentation). Augmentation with identifier-renamed data is skipped
because REVEAL's structural node features are invariant to identifier renaming.

Evaluates on all 8 robustness conditions.
Output: devign_full/reveal_aug_seedSEED_results.json
"""
...
# ---------------------------------------------------------------------------
# Model — identical to reveal_faithful / reveal_noid
# ---------------------------------------------------------------------------
...
class RevealGNN(nn.Module):
    def __init__(self, hidden=HIDDEN, num_blocks=NUM_BLOCKS, dropout=0.3):
        ...
        self.head = nn.Sequential(
            nn.Linear(hidden * 2, 128), nn.ReLU(), nn.Dropout(dropout), nn.Linear(128, 1),
        )
...
def main():
    ...
    print(f"[REVEAL-Aug seed={args.seed}] Device: {device}", flush=True)
    ...
    for p in aug_paths:
        if not p.exists():
            print(f"ERROR: Missing augmented data: {p}", flush=True)
            print("Run exp7_reveal_aug_prep.py first.", flush=True)
            sys.exit(1)

    # Load augmented training data (originals + deadcode + controlflow)
    print("Loading training data (3× augmentation)…", flush=True)
    orig_train = load_graphs(DEVIGN_INPUT / "originals_train"    / "train_GGNNinput.json")
    dead_train = load_graphs(DEVIGN_INPUT / "obf_deadcode_train" / "train_GGNNinput.json")
    cf_train   = load_graphs(DEVIGN_INPUT / "obf_controlflow_train" / "train_GGNNinput.json")
    valid_graphs = load_graphs(DEVIGN_INPUT / "originals_train"  / "valid_GGNNinput.json")

    train_graphs = orig_train + dead_train + cf_train
    print(f"  orig={len(orig_train)}  dead={len(dead_train)}  cf={len(cf_train)}", flush=True)
    print(f"  Total train: {len(train_graphs)}  Valid: {len(valid_graphs)}", flush=True)

    pos = sum(int(g.y.item()) for g in train_graphs)
    print(f"  Train pos={pos} ({100*pos/len(train_graphs):.1f}%)", flush=True)
    ...
    out = THESIS_ROOT / "devign_full" / f"reveal_aug_seed{args.seed}_results.json"
