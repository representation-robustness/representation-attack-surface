#!/usr/bin/env python3
"""
generate_roc_data_bigvul.py — Extract softmax probability scores for BigVul ROC curves.

Usage:
    CUDA_VISIBLE_DEVICES=1 /home/jesse/venvs/reveal310/bin/python3 ~/thesis/generate_roc_data_bigvul.py
"""
...
BV_SPLITS   = Path.home() / "thesis/bigvul/splits"
LMGGNN_DIR  = str(Path.home() / "vul-LMGGNN")
OUT_FILE    = THESIS_ROOT / "devign_full/roc_data_bigvul.json"
BATCH       = 128

print(f"Device: {DEVICE}")


def load_cpg_type(split_name):
    sys.path.insert(0, str(Path.home() / "vulgnn_devign"))
    from cpg_parser import load_split as _load
    graphs = _load(split_name, node_feats='type', dataset='bigvul')
    sys.path.pop(0)
    return graphs


def load_cpg_full(split_name):
    """Load CPG graphs via bigvul_cpg_parser (for REVEAL, LMGGNN, VulGNN)."""
    sys.path.insert(0, str(Path.home()))
    from bigvul_cpg_parser import load_split as _load
    graphs = _load(split_name)
    # keep bigvul_cpg_parser on path for downstream use
    return graphs
...
def extract_ecg_rgcn(graphs):
    ...
    ckpt = Path.home() / "ecgrgcn_bigvul_ckpts/ecgrgcn_bv_seed42.pt"
    model = ECGRGCNBigVul().to(DEVICE)
    model.load_state_dict(torch.load(ckpt, map_location=DEVICE, weights_only=False))
    return extract_probs_pyg(model, PyGLoader(graphs, batch_size=BATCH, shuffle=False))


def extract_angle(cpg_graphs_code):
    ...
    ckpt = Path.home() / "angle_bigvul_ckpts/angle_bv_seed42.pt"
...
def extract_vulgnn(graphs):
    ...
    ckpt = Path.home() / "vulgnn_bigvul_ckpts/vulgnn_bv_seed42.pt"
    state = torch.load(ckpt, map_location=DEVICE, weights_only=False)
...
def extract_reveal(graphs):
    ...
    sys.path.insert(0, str(Path.home()))
    from bigvul_cpg_parser import NUM_NODE_TYPES
    sys.path.pop(0)
    ...
    ckpt = Path.home() / "reveal_bigvul_ckpts/reveal_bv_seed42.pt"
    model = REVEALBigVul().to(DEVICE)
    model.load_state_dict(torch.load(ckpt, map_location=DEVICE, weights_only=False))
    return extract_probs_pyg(model, PyGLoader(graphs, batch_size=BATCH, shuffle=False))
...
def extract_lmggnn(graphs):
    ...
    ckpt_path = Path.home() / "vul-LMGGNN/data/model/lmggnn_bigvul_seed42.pt"
...
def extract_codebert(ckpt_path):
    ...
    test_recs = load_jsonl_bv(BV_SPLITS / "test.jsonl")
...
def extract_tfidf():
    ...
    tr_recs = load_jsonl_bv(BV_SPLITS / "train.jsonl")
    te_recs = load_jsonl_bv(BV_SPLITS / "test.jsonl")
...
def main():
    ...
    # Load CPG graphs
    print("\nLoading BigVul CPG test graphs...", flush=True)
    sys.path.insert(0, str(Path.home()))
    from bigvul_cpg_parser import load_split as bv_load
    cpg_graphs      = bv_load('test')
    cpg_code_graphs = bv_load('test', node_feats='code')
    print(f"  {len(cpg_graphs)} graphs loaded", flush=True)

    CB_CKPT  = THESIS_ROOT / "baselines/codebert/ckpts_multiseed/codebert_bigvul_seed42.pt"
    CT5_CKPT = THESIS_ROOT / "baselines/codebert/ckpts_codet5plus/codet5plus_bigvul_seed42.pt"
