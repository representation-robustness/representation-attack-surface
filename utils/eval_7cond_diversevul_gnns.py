"""
eval_7cond_diversevul_gnns.py — Evaluate GNN models on all 8 conditions for DiverseVul.

Models: VulGNN, ANGLE, ECG-RGCN, REVEAL
Checkpoints: ~/vulgnn_diversevul_ckpts/, ~/angle_diversevul_ckpts/,
             ~/ecgrgcn_diversevul_ckpts/, ~/reveal_sys_diversevul_ckpts/

Usage:
    CUDA_VISIBLE_DEVICES=X python eval_7cond_diversevul_gnns.py --model vulgnn
    CUDA_VISIBLE_DEVICES=X python eval_7cond_diversevul_gnns.py --model angle
    CUDA_VISIBLE_DEVICES=X python eval_7cond_diversevul_gnns.py --model ecgrgcn
    CUDA_VISIBLE_DEVICES=X python eval_7cond_diversevul_gnns.py --model reveal

Output: ~/thesis/devign_full/{model}_7cond_diversevul_results.json
"""
...
CKPT_DIRS = {
    'vulgnn':  os.path.expanduser('~/vulgnn_diversevul_ckpts'),
    'angle':   os.path.expanduser('~/angle_diversevul_ckpts'),
    'ecgrgcn': os.path.expanduser('~/ecgrgcn_diversevul_ckpts'),
    'reveal':  os.path.expanduser('~/reveal_sys_diversevul_fixed_ckpts'),
}

CKPT_PATTERNS = {
    'vulgnn':  'vulgnn_dv_seed{seed}.pt',
    'angle':   'angle_dv_seed{seed}.pt',
    'ecgrgcn': 'ecgrgcn_dv_seed{seed}.pt',
    'reveal':  'reveal_dv_fixed_seed{seed}.pt',
}
...
def build_model(model_name, vocab_size=None, embed_dim_w2v=None, pretrained=None):
    if model_name == 'vulgnn':
        ...
        return ANGLE(vocab_size=vocab_size, embed_dim=embed_dim_w2v, hidden=64,
                     pool_ratio=0.5, num_layers=3, dropout=0.1,
                     pretrained_emb=pretrained).to(DEVICE)
    elif model_name == 'ecgrgcn':
        return ECGRGCNDiverseVul(NUM_NODE_TYPES, embed_dim=32, hidden=128,
                                 num_layers=3, dropout=0.1).to(DEVICE)
    elif model_name == 'reveal':
        return REVEALDiverseVul(NUM_NODE_TYPES, embed_dim=32, hidden=200,
...
def run_model(model_name):
    print(f"\n{'='*60}")
    print(f"Model: {model_name.upper()}  |  Dataset: DiverseVul  |  Device: {DEVICE}")
    ...
    for seed in SEEDS:
        ...
        out = os.path.join(RESULTS_DIR, f'{model_name}_7cond_diversevul_seed{seed}_results.json')
    ...
    out = os.path.join(RESULTS_DIR, f'{model_name}_7cond_diversevul_results.json')
    with open(out, 'w') as f:
