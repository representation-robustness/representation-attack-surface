"""
CPG parser for ReVeal dataset — reads from ~/reveal_cpg/ instead of ~/bigvul_cpg/.
"""
import os, sys
import glob
import torch
...
REVEAL_CPG_ROOT = os.path.expanduser("~/reveal_cpg")


def load_split(split: str, node_feats: str = 'type', max_chunks: int = None) -> list:
    split_dir = os.path.join(REVEAL_CPG_ROOT, split)
