"""
angle_bigvul.py — ANGLE multi-seed training on Big-Vul.

Usage:
    CUDA_VISIBLE_DEVICES=3 python ~/angle_bigvul.py

Outputs:
    ~/thesis/devign_full/bigvul_angle_multiseed_results.json
"""
...
from bigvul_cpg_parser import load_split, tokenize_code
...
# Reuse Devign Word2Vec vocab (reasonable for code tokens)
...
CKPT_DIR    = os.path.expanduser('~/angle_bigvul_ckpts')
...
def load_code_graphs(split, vocab, max_seq_len=MAX_SEQ_LEN):
    """Load CPG pkls and tokenize node CODE properties for ANGLE."""
    import glob, pandas as pd
    from bigvul_cpg_parser import BIGVUL_CPG_ROOT
    sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "vulgnn"))
    from cpg_parser import cpg_to_graph
    # Re-load with 'code' node features
    graphs = load_split(split, node_feats='code')

    # Tokenize each graph's node tokens
    ...
    for g in graphs:
        ...
        for code_str in g.code_tokens:
            toks = tokenize_code(code_str, vocab, max_seq_len)
            tokens.append(toks)
...
def train_one_seed(seed, train_graphs, valid_graphs, test_graphs, vocab_size, embed_dim, pretrained):
    set_seed(seed)
    print(f"\n{'='*55}\n  ANGLE Big-Vul  Seed {seed}\n{'='*55}")
    ckpt = os.path.join(CKPT_DIR, f'angle_bv_seed{seed}.pt')
    ...
    out = os.path.join(RESULTS_DIR, f'bigvul_angle_seed{seed}_results.json')
...
def main():
    ...
    out = os.path.join(RESULTS_DIR, 'bigvul_angle_multiseed_results.json')
