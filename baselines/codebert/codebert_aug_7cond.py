#!/usr/bin/env python3
"""CodeBERT-Aug 7-condition eval: re-train with pairwise test splits.

Same training protocol as codebert_augmented_multiseed.py, but evaluates
all 7 obfuscation conditions: Ren, Dead, CF, R+D, R+CF, D+CF, Cmp.
Pairwise JSON files must exist in devign_full/.
"""
...
PAIRWISE_DATA_FILES = {
    "obf_ren_dead":    DEVIGN_ROOT / "obf_ren_dead_full_data_with_slices.json",
    "obf_ren_cf":      DEVIGN_ROOT / "obf_ren_cf_full_data_with_slices.json",
    "obf_dead_cf":     DEVIGN_ROOT / "obf_dead_cf_full_data_with_slices.json",
    "obf_compound":    DEVIGN_ROOT / "obf_compound_full_data_with_slices.json",
}
...
def train_one_seed(seed, tokenizer, train_recs, valid_recs, test_splits):
    ckpt_path = CKPT_DIR / f"codebert_aug_devign_seed{seed}.pt"

    set_seed(seed)
    print(f"\n{'='*55}\n  CodeBERT-Aug 7-cond  Seed {seed}\n{'='*55}", flush=True)
    ...
    torch.save({"model_state_dict": best_state, "best_val_f1": best_val_f1, "seed": seed},
               ckpt_path)
    ...
    out = DEVIGN_ROOT / f"codebert_aug_7cond_seed{seed}_results.json"
...
def main():
    ...
    sources = {}
    for key, path in TRAIN_DATA_FILES.items():
        with open(path) as f: data = json.load(f)
        sources[key] = {d["file_name"]: d for d in data}
    pairwise = {}
    for key, path in PAIRWISE_DATA_FILES.items():
        with open(path) as f: data = json.load(f)
        pairwise[key] = {d["file_name"]: d for d in data}
    ...
    test_splits = {"test": [orig_idx[n] for n in test_names if n in orig_idx]}
    for obf_key, src_key in [
        ...
        idx = sources[src_key]
        test_splits[obf_key] = [idx[n] for n in test_names if n in idx]
    for obf_key, src_key in [
        ("test_obf_ren_dead",  "obf_ren_dead"),
        ("test_obf_ren_cf",    "obf_ren_cf"),
        ("test_obf_dead_cf",   "obf_dead_cf"),
        ("test_obf_compound",  "obf_compound"),
    ]:
        idx = pairwise[src_key]
        test_splits[obf_key] = [idx[n] for n in test_names if n in idx]

    print(f"Augmented train={len(train_recs)} valid={len(valid_recs)}", flush=True)
