#!/usr/bin/env python3
"""CodeT5+ 220M 5-seed multiseed evaluation on ReVeal dataset."""
...
SPLITS_DIR  = THESIS_ROOT / "reveal_dataset" / "splits"
RESULTS_DIR = THESIS_ROOT / "devign_full"
CKPT_DIR    = SCRIPT_DIR / "ckpts_reveal_codet5plus"
...
def train_one_seed(seed, tokenizer, train_recs, valid_recs, test_sets):
    set_seed(seed)
    print(f"\n{'='*55}\n  CodeT5+ ReVeal  Seed {seed}\n{'='*55}", flush=True)
    ckpt = CKPT_DIR / f"codet5plus_reveal_seed{seed}.pt"
...
def main():
    ...
    agg = {"n_seeds": len(SEEDS), "seeds": SEEDS, "model": "CodeT5+", "dataset": "ReVeal"}
    for cond in conds:
    ...
    out = RESULTS_DIR / "reveal_codet5plus_multiseed_results.json"
