#!/usr/bin/env python3
"""CodeBERT 5-seed multiseed evaluation on DiverseVul (balanced subset)."""
...
CKPT_DIR    = SCRIPT_DIR / "ckpts_diversevul_codebert"
...
def train_one_seed(seed, tokenizer, train_recs, valid_recs, test_sets):
    set_seed(seed)
    print(f"\n{'='*55}\n  CodeBERT DiverseVul  Seed {seed}\n{'='*55}", flush=True)
    ckpt = CKPT_DIR / f"codebert_dv_seed{seed}.pt"
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for batch in train_loader:
            ...
            logits = model(iids, attention_mask=amsk).logits
...
def main():
    ...
    agg = {"n_seeds": len(SEEDS), "seeds": SEEDS, "model": "CodeBERT", "dataset": "DiverseVul"}
    for cond in conds:
    ...
    out = RESULTS_DIR / "diversevul_codebert_multiseed_results.json"
