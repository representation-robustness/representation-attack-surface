"""
ecgrgcn_bigvul.py — ECG RGCN (Relational GCN) on Big-Vul using CPG pkls.

Uses 2-edge CPG format (AST=0, CFG=1) instead of 5-edge GGNN format.
Architecture: 3-layer R-GCN + global mean+max pool + MLP head.

Usage:
    CUDA_VISIBLE_DEVICES=2 python ~/ecgrgcn_bigvul.py  (after VulGNN finishes)

Outputs:
    ~/thesis/devign_full/bigvul_ecgrgcn_multiseed_results.json
"""
...
CKPT_DIR    = os.path.expanduser('~/ecgrgcn_bigvul_ckpts')
...
class ECGRGCNBigVul(nn.Module):
    """R-GCN with 2 relation types (AST, CFG) for Big-Vul CPG data."""
...
def train_one_seed(seed, train_graphs, valid_graphs, test_graphs):
    set_seed(seed)
    print(f"\n{'='*55}\n  ECG RGCN Big-Vul  Seed {seed}\n{'='*55}")
    ckpt = os.path.join(CKPT_DIR, f'ecgrgcn_bv_seed{seed}.pt')
    ...
    model     = ECGRGCNBigVul(NUM_NODE_TYPES, EMBED_DIM, HIDDEN, NUM_LAYERS, DROPOUT).to(DEVICE)
    ...
    out = os.path.join(RESULTS_DIR, f'bigvul_ecgrgcn_seed{seed}_results.json')
...
def aggregate(all_results):
    ...
    agg.update({'n_seeds': len(all_results), 'seeds': [r['seed'] for r in all_results],
                'model': 'ECG RGCN', 'dataset': 'Big-Vul'})
    return agg
...
def main():
    ...
    out = os.path.join(RESULTS_DIR, 'bigvul_ecgrgcn_multiseed_results.json')
