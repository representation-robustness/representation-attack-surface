"""
vulgnn_bigvul.py — VulGNN multi-seed training on Big-Vul.

Usage:
    CUDA_VISIBLE_DEVICES=2 python ~/vulgnn_bigvul.py

Outputs:
    ~/thesis/devign_full/bigvul_vulgnn_multiseed_results.json
"""
...
CKPT_DIR    = os.path.expanduser('~/vulgnn_bigvul_ckpts')
...
def train_one_seed(seed, train_graphs, valid_graphs, test_graphs):
    set_seed(seed)
    print(f"\n{'='*55}\n  VulGNN Big-Vul  Seed {seed}\n{'='*55}")
    ckpt = os.path.join(CKPT_DIR, f'vulgnn_bv_seed{seed}.pt')
    ...
    out = os.path.join(RESULTS_DIR, f'bigvul_vulgnn_seed{seed}_results.json')
...
def aggregate(all_results):
    ...
    agg.update({'n_seeds': len(all_results), 'seeds': [r['seed'] for r in all_results],
                'model': 'VulGNN', 'dataset': 'Big-Vul'})
    return agg


def main():
    print(f"Device: {DEVICE}")
    print("Loading Big-Vul CPG splits (this may take a few minutes)...")
    t0 = time.time()
    ...
    out = os.path.join(RESULTS_DIR, 'bigvul_vulgnn_multiseed_results.json')
