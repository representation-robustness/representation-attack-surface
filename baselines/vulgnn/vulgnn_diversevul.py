"""
vulgnn_diversevul.py — VulGNN multi-seed training on DiverseVul (balanced subset).

Usage:
    CUDA_VISIBLE_DEVICES=X python ~/vulgnn_diversevul.py

Outputs:
    ~/thesis/devign_full/diversevul_vulgnn_multiseed_results.json
"""
...
CKPT_DIR    = os.path.expanduser('~/vulgnn_diversevul_ckpts')
...
def train_one_seed(seed, train_graphs, valid_graphs, test_graphs):
    set_seed(seed)
    print(f"\n{'='*55}\n  VulGNN DiverseVul  Seed {seed}\n{'='*55}")
    ckpt = os.path.join(CKPT_DIR, f'vulgnn_dv_seed{seed}.pt')
    ...
    model = VulGNN(num_node_types=NUM_NODE_TYPES, embed_dim=EMBED_DIM,
                   hidden=HIDDEN, num_layers=NUM_LAYERS,
                   num_edge_types=NUM_EDGE_TYPES, edge_dim=EDGE_DIM,
    ...
    out = os.path.join(RESULTS_DIR, f'diversevul_vulgnn_seed{seed}_results.json')
...
def aggregate(all_results):
    ...
    agg.update({'n_seeds': len(all_results), 'seeds': [r['seed'] for r in all_results],
                'model': 'VulGNN', 'dataset': 'DiverseVul'})
    return agg


def main():
    print(f"Device: {DEVICE}")
    print("Loading DiverseVul CPG splits (this may take a few minutes)...")
    t0 = time.time()
    ...
    out = os.path.join(RESULTS_DIR, 'diversevul_vulgnn_multiseed_results.json')
