"""
reveal_bigvul.py — REVEAL (GRU-based GNN) on Big-Vul using CPG pkls (AST+CFG).

Uses 2-edge CPG format (AST, CFG) instead of 5-edge GGNN format.
Node features: CPG node type embeddings (same as Devign REVEAL for node type branch).

Usage:
    CUDA_VISIBLE_DEVICES=5 python ~/reveal_bigvul.py

Outputs:
    ~/thesis/devign_full/bigvul_reveal_multiseed_results.json
"""
...
CKPT_DIR    = os.path.expanduser('~/reveal_bigvul_ckpts')
...
class REVEALBigVul(nn.Module):
    """REVEAL-style GRU graph NN adapted for CPG (2 edge types) on Big-Vul."""
    ...
    def forward(self, data):
        ...
        out = torch.cat([mean_p, max_p], dim=-1)
...
def train_one_seed(seed, train_graphs, valid_graphs, test_graphs):
    set_seed(seed)
    print(f"\n{'='*55}\n  REVEAL Big-Vul  Seed {seed}\n{'='*55}")
    ckpt = os.path.join(CKPT_DIR, f'reveal_bv_seed{seed}.pt')
    ...
    model = REVEALBigVul(NUM_NODE_TYPES, EMBED_DIM, HIDDEN, NUM_BLOCKS, STEPS_PER, DROPOUT).to(DEVICE)
    ...
    out = os.path.join(RESULTS_DIR, f'bigvul_reveal_seed{seed}_results.json')
...
def main():
    ...
    out = os.path.join(RESULTS_DIR, 'bigvul_reveal_multiseed_results.json')
